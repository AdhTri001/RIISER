...
class ChatBot:
    ...
    def __init__(self) -> None:
        self.model = load_model("data/chatbot/TensorBot_v2.h5")
        self.words, self.classes = pkload(open("data/chatbot/chatbot_dump_v2.pkl", "rb"))
        self.intents = jload(open("data/chatbot/intents.json", "r"))
        self.context = None
    ...
    def get_response(self, message: str) -> str:
        ...
        for _intent in _intents:
            if _intent.get("cont_get", None) == self.context:
                return self._process_intent(_intent, message)
        return self._process_intent(_intents[0], message)

    def _process_intent(self, intent, message):
        """
        ...
        if "func" in intent.keys():
            ...
            return "It is a function and functional responeses are under construction"
